...
public class DiscoverMovieViewModel extends ViewModel {
    ...
    private DiscoverSearchAPI discoverMovieAPI;
    private MutableLiveData<List<?>> listDiscoverMovie = new MutableLiveData<>();

    public void loadDiscoverMovie(View v, ImageView loadingImage, TextView loadingText, String sort_by, int year, int page_number) {
        ...
        // API
        discoverMovieAPI = new DiscoverSearchAPI();
        discoverMovieAPI.setContext(v.getContext());

        // Fetching Data
        CacheReq cacheReq = Utility.discoverSearch(urlApi, discoverMovieAPI, v, loadingImage, loadingText, listDiscoverMovie, "Movies");
        RequestQueue requestQueue = Volley.newRequestQueue(v.getContext());
        requestQueue.add(cacheReq);
    }

    public LiveData<List<?>> getDiscoverMovie() {
        return listDiscoverMovie;
    }

    public DiscoverSearchAPI getDiscoverMovieAPI() {
        return discoverMovieAPI;
    }
}
